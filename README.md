...
| 164-assertion end-to-end suite over recorded API fixtures, on a frozen clock in a pinned timezone | Testing and verification discipline, including the test that decays on its own |
...
164 assertions against recorded API responses, so the suite needs no network and does not change
